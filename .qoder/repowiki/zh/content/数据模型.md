...
## 更新摘要
**变更内容**
- 类型定义更新：ProjectMeta 接口新增 preview_images 字段，支持项目预览图片数组
- 数据库模式扩展：新增 preview_images JSONB 列，支持项目预览图片存储
- 项目管理功能增强：支持项目预览图片的获取和管理
- API 接口更新：项目列表和详情接口返回 preview_images 字段
- TypeScript 兼容性：确保类型定义与数据库模式保持同步
...
- ProjectMeta：项目元数据，包含项目基本信息、缩略图 URL 和预览图片数组
- ProjectDetail：项目详情，继承 ProjectMeta 并包含扩展快照数据
...
- 项目管理：lib/project-service.ts（项目 CRUD 操作）
- 数据库模式：supabase/schema.sql（用户表、项目表和验证码表）
...
- 页面布局：app/page.tsx、app/login/page.tsx、app/register/page.tsx、app/profile/page.tsx、app/projects/page.tsx
- API路由：app/api/auth/*（认证相关接口）、app/api/user/*（用户资料和头像接口）、app/api/projects/*（项目管理接口）
...
A4["supabase/schema.sql<br/>用户表/验证码表/项目表"]
end
subgraph "类型定义"
T1["lib/types.ts<br/>CanvasItem / Message / StoredRef / Marker / User / ProjectMeta / ProjectDetail"]
end
subgraph "状态管理"
S1["lib/store.ts<br/>useAppStore<br/>持久化切片<br/>标记状态管理<br/>项目状态管理"]
end
subgraph "项目管理"
PS["lib/project-service.ts<br/>项目 CRUD 操作<br/>自动保存"]
end
subgraph "文件校验"
...
P2["app/projects/page.tsx<br/>项目管理页面"]
end
subgraph "认证页面"
P3["app/login/page.tsx<br/>密码/验证码登录"]
P4["app/register/page.tsx<br/>注册页面"]
end
subgraph "API路由"
R1["app/api/auth/*<br/>认证接口"]
R2["app/api/user/*<br/>用户资料接口"]
R3["app/api/projects/*<br/>项目管理接口"]
end
...
- [supabase/schema.sql:1-55](file://supabase/schema.sql#L1-L55)
- [lib/types.ts:1-87](file://lib/types.ts#L1-L87)
...
- [app/projects/page.tsx:1-200](file://app/projects/page.tsx#L1-L200)
...
- [app/api/projects/[id]/route.ts:1-276](file://app/api/projects/[id]/route.ts#L1-L276)
...
## 核心组件
本节对八个核心数据类型进行逐项说明，包括字段定义、类型约束、业务规则与典型用法。
...
- Message（聊天消息）
  ...
  - 使用示例路径
    ...
    - 历史截断逻辑：[lib/store.ts:276-280](file://lib/store.ts#L276-L280)
...
- AppState（应用状态）
  - 字段与约束
    ...
    - user：User 或空，当前登录用户
    - recentProjects：ProjectMeta 数组，最近打开的项目
    - isRestoringProject：布尔，项目恢复状态
  ...
  - 使用示例路径
    ...
    - 清空画布并重置编辑态：[lib/store.ts:188-215](file://lib/store.ts#L188-L215)
    ...
    - 设置最近项目：[lib/store.ts:134-135](file://lib/store.ts#L134-L135)
...
- ProjectMeta（项目元数据）
  - 字段与约束
    - id：字符串，项目唯一标识
    - name：字符串，项目名称
    - thumbnail_url：字符串或空，项目缩略图 URL
    - preview_images：字符串数组，项目预览图片 URL 数组
    - created_at：字符串，创建时间戳
    - updated_at：字符串，更新时间戳
  - 业务规则
    - preview_images 字段支持项目预览图片数组
    - 预览图片 URL 数组可能为空数组
    - 用于项目列表展示和快速预览
  - 使用示例路径
    - 项目列表获取：[app/api/projects/route.ts:33](file://app/api/projects/route.ts#L33)
    - 项目详情获取：[app/api/projects/[id]/route.ts:42](file://app/api/projects/[id]/route.ts#L42)
    - 项目创建：[app/api/projects/route.ts:108](file://app/api/projects/route.ts#L108)

- ProjectDetail（项目详情）
  - 字段与约束
    - 继承 ProjectMeta 的所有字段
    - snapshot：ExtendedSnapshot 或空，项目快照数据
    - snapshot_url：字符串或空，项目快照存储 URL
  - 业务规则
    - 继承 ProjectMeta 的所有业务规则
    - snapshot 包含 tldraw 画布状态和lovart扩展数据
    - 支持从 Storage 或 JSONB 加载快照
  - 使用示例路径
    - 项目详情加载：[app/api/projects/[id]/route.ts:101](file://app/api/projects/[id]/route.ts#L101)
    - 快照加载：[app/api/projects/[id]/route.ts:65](file://app/api/projects/[id]/route.ts#L65)
...
- [lib/types.ts:36-42](file://lib/types.ts#L36-L42)
- [lib/types.ts:44-50](file://lib/types.ts#L44-L50)
- [lib/types.ts:52-58](file://lib/types.ts#L52-L58)
- [lib/types.ts:60-67](file://lib/types.ts#L60-L67)
- [lib/types.ts:69-72](file://lib/types.ts#L69-L72)
...
- [app/api/projects/route.ts:33](file://app/api/projects/route.ts#L33)
- [app/api/projects/[id]/route.ts:42](file://app/api/projects/[id]/route.ts#L42)
- [app/api/projects/[id]/route.ts:101](file://app/api/projects/[id]/route.ts#L101)
...
- [lib/store.ts:188-215](file://lib/store.ts#L188-L215)
- [lib/store.ts:276-280](file://lib/store.ts#L276-L280)
...
- 验证码系统支持短信发送和本地开发模式
- 用户资料功能支持昵称设置和头像上传管理
- 项目管理通过 ProjectMeta 和 ProjectDetail 支持项目预览图片展示
...
Profile->>DB : 更新用户资料
DB-->>Profile : 返回更新结果
UI->>Projects : 获取项目列表/详情
Projects->>DB : 查询项目元数据和预览图片
DB-->>Projects : 返回项目数据
Projects-->>UI : 展示项目预览图片
```
...
- 字段与含义
  ...
  - uploading/placeholder/loading：上传与生成状态
...
AppState --> Marker : "包含"
AppState --> User : "包含"
AppState --> ProjectMeta : "包含"
ProjectDetail --> ProjectMeta : "继承"
```
...
- 关键处理逻辑
  ...
  - 用于项目列表展示和快速预览
- API 集成
  - 项目列表接口返回包含 preview_images 的 ProjectMeta
  - 项目详情接口返回包含 preview_images 的 ProjectDetail
  - 支持从数据库查询预览图片字段

```mermaid
flowchart TD
Query["查询项目列表"] --> SelectFields["SELECT id, name, thumbnail_url, preview_images, created_at, updated_at"]
SelectFields --> ReturnMeta["返回 ProjectMeta[]"]
ReturnMeta --> UI["UI 展示项目预览图片"]
UI --> End(["完成"])
```
...
- 关键处理逻辑
  ...
  - snapshot 包含 tldraw 画布状态和lovart扩展数据
  - 支持从 Storage 或 JSONB 加载快照
  - 支持 snapshot_url 字段的可选性
- 快照加载策略
  - 优先从 Storage 加载 snapshot_url
  - 失败时降级到 project_snapshots 表的 JSONB 数据
  - 无快照时保持 null

```mermaid
flowchart TD
Start(["加载项目详情"]) --> CheckSnapshotURL{"有 snapshot_url？"}
CheckSnapshotURL --> |是| LoadFromStorage["从 Storage 加载快照"]
LoadFromStorage --> StorageSuccess{"加载成功？"}
StorageSuccess --> |是| UseStorage["使用 Storage 快照"]
StorageSuccess --> |否| LoadFromJSONB["从 JSONB 表加载"]
CheckSnapshotURL --> |否| LoadFromJSONB
LoadFromJSONB --> Found{"找到 JSONB 快照？"}
Found --> |是| UseJSONB["使用 JSONB 快照"]
Found --> |否| NoSnapshot["保持 snapshot=null"]
UseStorage --> BuildResponse["构建 ProjectDetail"]
UseJSONB --> BuildResponse
NoSnapshot --> BuildResponse
BuildResponse --> End(["完成"])
```

**图表来源**
- [app/api/projects/[id]/route.ts:65](file://app/api/projects/[id]/route.ts#L65)
- [app/api/projects/[id]/route.ts:79](file://app/api/projects/[id]/route.ts#L79)
- [app/api/projects/[id]/route.ts:101](file://app/api/projects/[id]/route.ts#L101)
...
## 依赖分析
- 类型依赖
  - CanvasItem/Message/StoredRef/Marker/User/ProjectMeta/ProjectDetail 定义于 lib/types.ts
  - VerificationCode 定义于 supabase/schema.sql
  - useAppStore 在 lib/store.ts 中消费上述类型
  - ProjectMeta/ProjectDetail 在 lib/project-service.ts 中使用
- 认证依赖
...
- UI 依赖
  ...
  - 项目管理页面依赖 ProjectMeta 类型与项目 API
...
Types --> ProjectService["lib/project-service.ts"]
...
- [app/api/projects/*:1-276](file://app/api/projects/[id]/route.ts#L1-L276)
...
- 存储与序列化
  ...
  - 项目状态通过 setRecentProjects 动作更新，避免全量重渲染
- 认证性能
  - JWT Token 缓存避免频繁解密
  - 数据库查询使用索引优化（users.phone、verification_codes.phone_code、verification_codes.expires_at）
  - 验证码发送频率限制减少数据库压力
  - 个人资料接口使用 select 指定字段，减少网络传输
  - 项目 API 查询使用精确字段选择，减少网络传输
- 数据库优化
  - 创建复合索引优化验证码查询性能
  - 使用 UUID 作为主键提升并发性能
  - 合理的时间戳字段设计支持高效的过期查询
  - 用户资料查询使用索引优化
  - 项目查询使用索引优化，支持按用户 ID 和更新时间排序
  - 项目预览图片字段支持 JSONB 查询和存储
...
- 历史溢出
  - 现象：超过 50 条后最旧消息被丢弃
  - 参考路径：[lib/store.ts:276-280](file://lib/store.ts#L276-L280)
- 标记数量限制
...
- 标记渲染异常
  - 现象：标记不显示或位置错误
  - 排查：确认 tldraw editor 正常运行，检查 shape 辨别计算
...
- 项目列表加载失败
  - 现象：项目列表为空或加载错误
  - 排查：检查数据库连接和项目查询权限
  - 参考路径：[app/api/projects/route.ts:37-43](file://app/api/projects/route.ts#L37-L43)
- 项目详情加载失败
  - 现象：项目详情页面无法加载或快照加载失败
  - 排查：检查项目 ID 权限和快照存储状态
  - 参考路径：[app/api/projects/[id]/route.ts:46-51](file://app/api/projects/[id]/route.ts#L46-L51)
- 预览图片显示异常
  - 现象：项目预览图片不显示或加载失败
  - 排查：检查 preview_images 字段数据和 CDN 链接有效性
  - 参考路径：[app/api/projects/route.ts:48](file://app/api/projects/route.ts#L48)
- 快照加载降级
  - 现象：从 Storage 加载快照失败，自动降级到 JSONB
  - 排查：检查 Storage 配置和 JSONB 数据完整性
  - 参考路径：[app/api/projects/[id]/route.ts:72-74](file://app/api/projects/[id]/route.ts#L72-L74)
- 测试验证
...
- [app/api/auth/me/route.ts:29-31](file://app/api/auth/me/route.ts#L29-L31)
- [app/api/projects/route.ts:37-43](file://app/api/projects/route.ts#L37-L43)
- [app/api/projects/[id]/route.ts:46-51](file://app/api/projects/[id]/route.ts#L46-L51)
- [app/api/projects/route.ts:48](file://app/api/projects/route.ts#L48)
- [app/api/projects/[id]/route.ts:72-74](file://app/api/projects/[id]/route.ts#L72-L74)
...
## 结论
Loveart 的数据模型围绕 AppState 组织，通过 CanvasItem、Message、StoredRef、Marker、User、ProjectMeta、ProjectDetail 七类实体支撑"创作+编辑+标记+用户资料+项目管理"的核心流程。新增的 ProjectMeta 和 ProjectDetail 类型通过 preview_images 字段支持项目预览图片展示，增强了项目的可视化管理和快速预览能力。

类型定义清晰、状态更新可预期、持久化策略合理，配合严格的文件校验、友好的 UI 反馈和强大的标记系统，形成完整的端到端体验。新增的用户资料功能通过独立的 API 接口实现，支持昵称设置和头像上传，进一步增强了平台的社交化和个性化特性。

数据库模式扩展增加了 preview_images JSONB 列，支持项目预览图片的存储和查询，为项目管理功能提供了数据基础。项目快照系统支持从 Storage 和 JSONB 两种方式加载，确保了数据的可靠性和兼容性。

建议在扩展新功能时遵循现有模式：先定义类型与约束，再在 store 中提供动作，最后在 UI 中消费状态并处理副作用。对于认证相关的新功能，应特别注意安全性考虑和性能优化。项目管理功能的引入为平台增加了更多协作和分享的可能性，建议后续可以考虑添加项目分享、团队协作等功能。
...
- 类型安全与序列化
  ...
  - 项目状态通过 setRecentProjects 动作更新，支持最近项目管理
- 使用示例索引
  ...
  - 项目详情获取：[app/api/projects/[id]/route.ts:42](file://app/api/projects/[id]/route.ts#L42)
  - 项目创建：[app/api/projects/route.ts:108](file://app/api/projects/route.ts#L108)
  - 项目删除：[app/api/projects/[id]/route.ts:189](file://app/api/projects/[id]/route.ts#L189)
  - 项目名称更新：[app/api/projects/[id]/route.ts:254](file://app/api/projects/[id]/route.ts#L254)
- 页面布局与容器
  ...
  - 项目管理页面：[app/projects/page.tsx:162-200](file://app/projects/page.tsx#L162-L200)
- 数据库索引优化
  - users.phone：加速用户查询
  - verification_codes.phone_code：加速验证码验证
  - verification_codes.expires_at：加速过期记录清理
  - projects.user_id：加速项目查询
  - projects.updated_at：加速项目排序
  - projects.preview_images：支持预览图片字段查询（如需）
  - project_snapshots.project_id：加速快照查询
  - project_snapshots.version：加速快照版本查询