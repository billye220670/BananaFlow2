...
## 更新摘要
**变更内容**
- 新增用户认证系统，包含用户表(users)和验证码表(verification_codes)
- 实现手机号唯一性约束和密码哈希存储机制
- 集成验证码发送、验证和过期管理功能
- 添加JWT认证中间件和Cookie管理机制
- 支持密码登录和验证码登录两种认证方式
...
- User：用户实体，包含手机号、密码哈希、昵称等认证相关信息
...
- 页面布局：app/page.tsx、app/login/page.tsx、app/register/page.tsx
- API路由：app/api/auth/*（认证相关接口）
...
subgraph "认证页面"
P1["app/login/page.tsx<br/>密码/验证码登录"]
P2["app/register/page.tsx<br/>注册页面"]
end
subgraph "API路由"
...
- [app/api/auth/signin/route.ts:1-92](file://app/api/auth/signin/route.ts#L1-L92)
- [app/api/auth/signin-sms/route.ts:1-94](file://app/api/auth/signin-sms/route.ts#L1-L94)
- [app/api/auth/signup/route.ts:1-120](file://app/api/auth/signup/route.ts#L1-L120)
...
## 核心组件
本节对六个核心数据类型进行逐项说明，包括字段定义、类型约束、业务规则与典型用法。
...
- User（用户实体）
  - 字段与约束
    ...
    - password_hash：字符串或空，密码哈希值，长度不超过255
    - nickname：字符串，昵称，长度不超过100
    - created_at：时间戳，默认当前时间
  - 业务规则
    - phone 字段具有唯一性约束，确保手机号不重复
    - password_hash 可为空，支持验证码登录方式
    - 支持昵称字段，可为空表示未设置
    - created_at 字段自动记录创建时间
  - 使用示例路径
    - 用户查询：[app/api/auth/signin/route.ts:36-49](file://app/api/auth/signin/route.ts#L36-L49)
    - 用户创建：[app/api/auth/signin-sms/route.ts:54-71](file://app/api/auth/signin-sms/route.ts#L54-L71)
    - 用户信息获取：[app/api/auth/me/route.ts:25-32](file://app/api/auth/me/route.ts#L25-L32)
...
- [app/api/auth/signin/route.ts:36-49](file://app/api/auth/signin/route.ts#L36-L49)
- [app/api/auth/signin-sms/route.ts:54-71](file://app/api/auth/signin-sms/route.ts#L54-L71)
- [app/api/auth/me/route.ts:25-32](file://app/api/auth/me/route.ts#L25-L32)
...
**图表来源**
- [app/api/auth/signin/route.ts:76-77](file://app/api/auth/signin/route.ts#L76-L77)
- [app/api/auth/signin-sms/route.ts:56-70](file://app/api/auth/signin-sms/route.ts#L56-L70)
- [lib/auth.ts:13-28](file://lib/auth.ts#L13-L28)

**章节来源**
- [supabase/schema.sql:1-8](file://supabase/schema.sql#L1-L8)
...
**章节来源**
- [supabase/schema.sql:10-18](file://supabase/schema.sql#L10-L18)
- [lib/sms.ts:43-115](file://lib/sms.ts#L43-L115)

## 依赖分析
- 类型依赖
  - CanvasItem/Message/StoredRef/Marker/State 定义于 lib/types.ts
  - User/VerificationCode 定义于 supabase/schema.sql
...
- [app/login/page.tsx:1-233](file://app/login/page.tsx#L1-L233)
- [app/register/page.tsx:1-199](file://app/register/page.tsx#L1-L199)
- [app/api/auth/*:1-92](file://app/api/auth/signin/route.ts#L1-L92)
...
## 结论
Loveart 的数据模型围绕 AppState 组织，通过 CanvasItem、Message、StoredRef、Marker 四类实体支撑"创作+编辑+标记"的核心流程。新增的用户认证系统通过 JWT Cookie 机制实现了安全的会话管理，支持手机号唯一性约束、密码哈希存储和验证码索引优化。类型定义清晰、状态更新可预期、持久化策略合理，配合严格的文件校验、友好的 UI 反馈和强大的标记系统，形成完整的端到端体验。

认证系统的引入增强了平台的安全性和用户体验，通过手机号唯一性约束确保用户身份的唯一性，通过密码哈希存储保护用户隐私，通过验证码索引优化提升了系统性能。标记功能通过 tldraw 集成，提供了直观的画布元素注释能力，进一步增强了协作和创作体验。

建议在扩展新功能时遵循现有模式：先定义类型与约束，再在 store 中提供动作，最后在 UI 中消费状态并处理副作用。对于认证相关的新功能，应特别注意安全性考虑和性能优化。
