...
**更新** 新增 FAL API 密钥持久化存储功能，支持用户自定义 API 密钥与代理路由的灵活认证机制。
...
- [app/api/fal/proxy/route.ts:1-18](file://app/api/fal/proxy/route.ts#L1-L18)
...
**更新** 新增 FAL API 密钥管理功能，支持用户自定义密钥与环境变量的双重认证机制。

**章节来源**
- [lib/store.ts:19-385](file://lib/store.ts#L19-L385)
- [lib/types.ts:1-49](file://lib/types.ts#L1-L49)
- [lib/fal.ts:1-107](file://lib/fal.ts#L1-L107)
...
**图表来源**
- [lib/store.ts:244-265](file://lib/store.ts#L244-L265)
- [lib/fal.ts:34-106](file://lib/fal.ts#L34-L106)
- [app/api/fal/proxy/route.ts:1-18](file://app/api/fal/proxy/route.ts#L1-L18)
...
- 已持久化切片（PersistedSlice）
  ...
  - projectName：项目名称，默认为 'Untitled'。
  - **新增** falApiKey：FAL API 密钥，支持用户自定义密钥存储。
- 会话切片（SessionSlice）
  - canvasItems：画布元素列表（含占位符、上传中状态）。
  - isEditingMode / editingTarget：编辑模式与当前目标。
  - isLoading：全局加载状态。
  - **新增** editor / selectedShapeIds：tldraw 集成支持。
  - **新增** isChatOpen / chatPanelWidth：聊天面板状态与宽度。
  - **新增** markers / activeTool：标记工具状态。
- 动作（Actions）
  - 画布：添加/更新/移除/清空；编辑模式切换与目标更新。
  - 参考图：添加/更新/移除；上传成功后回填远端地址。
  - 聊天：追加消息并裁剪历史。
  - 加载：设置加载状态。
  - **新增** tldraw 集成：设置编辑器实例与选择形状。
  - **新增** 项目名称：设置项目名称。
  - **新增** FAL API 密钥：设置用户自定义 API 密钥。
  - **新增** 聊天面板：切换、打开、关闭、清空历史、设置宽度。
  - **新增** 标记工具：激活工具、添加/移除标记、清理标记。
...
**图表来源**
- [lib/store.ts:66-86](file://lib/store.ts#L66-L86)
- [lib/store.ts:21-55](file://lib/store.ts#L21-L55)

**章节来源**
- [lib/store.ts:66-385](file://lib/store.ts#L66-L385)

### 数据模型详解
- CanvasItem
  - 字段：id、显示 URL、FAL CDN URL、坐标、尺寸、上传状态、占位符标记、参考图片数组、自然宽高、文件名。
  - 使用场景：画布元素渲染、拖拽、缩放、下载、清理。
- Message
  - 字段：id、角色（用户/助手）、内容、可选结果图 URL、时间戳、加载状态。
  - 使用场景：消息历史展示、结果反馈。
- StoredRef
  - 字段：id、本地对象 URL、FAL 远端 URL、名称、上传状态。
  - 使用场景：参考图上传预览、上传完成后的远端回填。
- **新增** Marker
  - 字段：id、所属 CanvasItem 的 id、全局序号 1-8、相对图片的 X/Y 位置（0-1）。
  - 使用场景：在画布元素上添加标记点。
...
**图表来源**
- [lib/types.ts:18-49](file://lib/types.ts#L18-L49)
...
- 中间件配置
  ...
  - 序列化策略：partialize 仅持久化 chatHistory、projectName 和 falApiKey。
- 行为特征
  - 启动时从持久化恢复聊天历史、项目名称和 API 密钥。
  - 会话内状态（画布、编辑态、加载态、tldraw 集成、聊天面板、标记工具）不写入持久化，保持纯净会话体验。

**更新** 新增 falApiKey 字段的持久化，支持用户自定义 FAL API 密钥的跨会话保存。

```mermaid
flowchart TD
Start(["应用启动"]) --> Load["读取持久化存储"]
Load --> Parse{"解析成功？"}
Parse --> |是| Restore["恢复 chatHistory, projectName, falApiKey"]
Parse --> |否| Empty["初始化空历史和默认密钥"]
...
**图表来源**
- [lib/store.ts:368-385](file://lib/store.ts#L368-L385)

**章节来源**
- [lib/store.ts:7-19](file://lib/store.ts#L7-L19)
- [lib/store.ts:368-385](file://lib/store.ts#L368-L385)
...
- [lib/store.ts:112-154](file://lib/store.ts#L112-L154)
- [lib/fal.ts:104-106](file://lib/fal.ts#L104-L106)
...
**图表来源**
- [lib/store.ts:244-248](file://lib/store.ts#L244-L248)
...
- [lib/store.ts:194-242](file://lib/store.ts#L194-L242)
- [lib/fal.ts:104-106](file://lib/fal.ts#L104-L106)
...
### FAL API 密钥管理
- 密钥存储
  - 用户可以在应用中设置自定义 FAL API 密钥，密钥通过 setFalApiKey 动作存储到持久化状态。
  - 密钥在 localStorage 中持久化，支持跨会话使用。
- 请求认证
  - FAL 客户端通过 requestMiddleware 检索 store 中的 falApiKey。
  - 如果存在用户自定义密钥，则通过 'x-fal-key' 请求头传递给代理路由。
  - 代理路由优先使用请求头中的自定义密钥，否则回退到环境变量中的 FAL_KEY。
- 代理路由
  - 支持两种认证方式：请求头中的自定义密钥或环境变量中的默认密钥。
  - 提供灵活的密钥管理机制，满足不同使用场景的需求。
...
participant FAL as "FAL 客户端"
participant Proxy as "FAL 代理路由"
User->>UI : 输入 FAL API 密钥
UI->>Store : setFalApiKey(密钥)
Store-->>UI : 更新 falApiKey 状态
UI->>FAL : 发起 API 请求
FAL->>Store : 获取 falApiKey
Store-->>FAL : 返回用户密钥
FAL->>Proxy : 发送请求头 x-fal-key
Proxy-->>FAL : 验证并处理请求
```
...
**章节来源**
- [lib/store.ts:66-94](file://lib/store.ts#L66-L94)
- [lib/store.ts:264-265](file://lib/store.ts#L264-L265)
- [lib/fal.ts:6-20](file://lib/fal.ts#L6-L20)
- [app/api/fal/proxy/route.ts:4-17](file://app/api/fal/proxy/route.ts#L4-L17)
...
- 副作用处理
  ...
  - **新增** tldraw 集成：编辑器实例与选择状态的同步管理。
  - **新增** 标记工具：标记的添加、移除与重新编号。
...
- **新增** tldraw 集成优化
  - 编辑器实例的懒加载与状态同步，避免不必要的重渲染。
  - 标记工具的批量更新与版本控制，减少 UI 刷新次数。
...
**章节来源**
- [components/canvas/CanvasArea.tsx:123-130](file://components/canvas/CanvasArea.tsx#L123-L130)
- [components/canvas/CanvasArea.tsx:158-167](file://components/canvas/CanvasArea.tsx#L158-L167)
...
- 对外部服务调用进行统一错误处理与回退。
- **新增** API 密钥管理最佳实践
  - 优先使用用户自定义密钥，提供更好的隐私保护。
  - 密钥存储应加密处理，避免明文存储风险。
  - 提供密钥验证机制，确保密钥有效性。

**章节来源**
- [lib/store.ts:244-385](file://lib/store.ts#L244-L385)
...
- 通过动作函数日志定位异步流程问题。
- **新增** FAL API 密钥调试
  - 检查 localStorage 中的 falApiKey 是否正确存储。
  - 验证请求头中 x-fal-key 是否正确传递。
  - 确认代理路由的密钥优先级逻辑。
...
- **新增** FAL API 密钥问题
  - 现象：API 调用失败或权限不足。
  - 排查：确认用户自定义密钥是否正确设置；检查请求头是否正确传递；验证代理路由的密钥优先级。
...
## 结论
本状态管理系统以 Zustand 为核心，清晰划分持久化与会话状态，围绕 CanvasItem、Message、StoredRef 构建完整的数据模型。通过动作函数统一管理副作用，UI 组件以选择器订阅实现高效渲染。配合持久化中间件与安全存储包装，系统在功能完整性与运行稳定性之间取得平衡。

**更新** 新增的 FAL API 密钥持久化功能进一步增强了系统的灵活性与安全性，支持用户自定义密钥与代理路由的双重认证机制。建议后续引入版本迁移与更细粒度的性能监控，持续提升可维护性与用户体验。

## 附录
- 文件清单与用途概览
  - lib/store.ts：Zustand Store 定义与持久化配置。
  - lib/types.ts：CanvasItem、Message、StoredRef、Marker 数据模型。
  - lib/fal.ts：FAL 图像生成/编辑/上传客户端，支持自定义 API 密钥。
  ...
  - app/api/fal/proxy/route.ts：FAL 服务代理路由，支持自定义密钥认证。
