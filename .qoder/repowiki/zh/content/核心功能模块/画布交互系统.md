...
## 更新摘要
**变更内容**
- 新增占位符闪烁加载动画系统，提供视觉反馈的加载状态
- 实现智能定位算法，精确计算编辑面板和标记的位置
- 增强占位符管理功能，包含保护机制防止尺寸覆盖
- 优化 tldraw 编辑器集成，提升性能和稳定性
- 改进注释覆盖层和标记覆盖层的渲染性能
...
**更新** 占位符到图像转换逻辑得到重大修复，确保从 FAL API 获取正确的宽高值，避免使用 tldraw 形状的旧尺寸属性。

**新增** 占位符闪烁加载动画系统：系统新增了 `PlaceholderShimmerOverlay` 组件，为占位符节点提供闪烁的加载动画效果，通过 CSS `@keyframes shimmer` 实现平滑的视觉反馈。

**新增** 智能定位算法：在 `InlineEditPanel` 中实现了精确的坐标转换算法，使用 `pageToScreen` 和 `pageToViewport` 方法确保编辑面板和标记在不同缩放级别下都能准确定位。

**新增** 增强的占位符管理功能：系统实现了保护机制，通过 `recentlyTransitionedRef` 和 `placeholderIdsRef` 状态管理，防止占位符到图像的转换过程中出现尺寸覆盖问题。
...
- [CanvasArea.tsx:1-1728](file://components/canvas/CanvasArea.tsx#L1-L1728)
...
**新增** 占位符闪烁加载动画系统：新增 `PlaceholderShimmerOverlay` 组件，为占位符节点提供闪烁的加载动画效果。
...
**章节来源**
- [CanvasArea.tsx:584-1728](file://components/canvas/CanvasArea.tsx#L584-L1728)
...
**图表来源**
- [CanvasArea.tsx:585-647](file://components/canvas/CanvasArea.tsx#L585-L647)
...
**图表来源**
- [CanvasArea.tsx:1058-1119](file://components/canvas/CanvasArea.tsx#L1058-L1119)
...
**更新** 占位符到图像转换逻辑得到重大修复，确保从 FAL API 获取正确的宽高值，避免使用 tldraw 形状的旧尺寸属性。这一修复显著提升了图像尺寸处理的准确性，为用户提供了更精确的显示效果。

**新增** 占位符闪烁加载动画系统：系统新增了 `PlaceholderShimmerOverlay` 组件，为占位符节点提供闪烁的加载动画效果，通过 CSS `@keyframes shimmer` 实现平滑的视觉反馈，提升了用户交互体验。

**新增** 智能定位算法：在 `InlineEditPanel` 中实现了精确的坐标转换算法，使用 `pageToScreen` 和 `pageToViewport` 方法确保编辑面板和标记在不同缩放级别下都能准确定位，提升了系统的可用性。
