...
## 更新摘要
**变更内容**
- 修复了 CanvasArea.tsx 中的占位符到图像转换逻辑，确保从 FAL API 获取正确的宽高值
- 改进了尺寸处理准确性，使用 CanvasItem 的最新尺寸而非 tldraw 形状的旧尺寸
- 新增了对 FAL API 两种不同返回数据类型的智能处理支持
- 增强了占位符转换过程中的尺寸同步机制
...
CheckState --> |是| GetOldShape[获取旧的geo形状]
...
5. **占位符转换尺寸丢失**：从 FAL API 获取的尺寸未正确应用到转换后的图像
