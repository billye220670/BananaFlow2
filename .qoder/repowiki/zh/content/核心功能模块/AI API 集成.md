...
## 更新摘要
**所做更改**
- 动态端点选择机制：`generateImage` 函数现在根据是否存在参考图动态选择 `nano-banana-2` 或 `nano-banana-2/edit` 端点
- 增强的错误处理和日志记录：所有 API 调用都增加了详细的调试日志和错误处理，包括时间戳和参数摘要
- 改进的文件上传功能：`uploadFile` 函数现在包含完整的错误日志记录和文件信息输出
- 完整的图像元数据返回：`generateImage` 函数返回 `{ url, width, height }` 结构，替代了之前的纯 URL 返回
- 优化的尺寸检测：支持 FAL API 返回的宽高信息，若缺失则通过图片加载获取
...
**更新** 本次更新实现了多项重大增强：动态端点选择机制、增强的错误处理和日志记录、改进的文件上传功能。`generateImage` 函数现在返回完整的图像元数据（包含 URL、宽度、高度），而非仅返回 URL。这一变更显著改善了图像处理管道的精度和性能，使得应用能够基于实际图像尺寸进行优化显示和文件命名。
...
- [app/api/fal/proxy/route.ts:1-51](file://app/api/fal/proxy/route.ts#L1-L51)
...
## 核心组件
- FAL 客户端封装：统一配置代理地址、封装生成与编辑函数、提供文件上传能力，支持动态端点选择和自动图像尺寸检测
- 代理路由：服务端转发 FAL 请求，隐藏密钥，限制跨域风险，提供增强的日志记录和错误处理
...
**更新** 新增动态端点选择机制，`generateImage` 函数根据是否存在参考图自动选择合适的模型端点。增强的错误处理和日志记录系统为每个 API 调用提供详细的调试信息。完整的图像元数据返回机制支持基于实际图像尺寸的优化显示。
...
**更新** 重大架构变更：`generateImage` 函数现在返回完整的图像元数据，包括 URL、宽度和高度信息。动态端点选择机制根据输入参数自动选择最适合的模型。增强的日志记录系统为每个 API 调用提供详细的调试信息。
...
- [app/api/fal/proxy/route.ts:4-18](file://app/api/fal/proxy/route.ts#L4-L18)
...
- 动态端点选择：根据是否存在参考图自动选择 `nano-banana-2` 或 `nano-banana-2/edit` 端点
- 图像生成：构造基础输入参数，合并提示词与可选参考图 URL，支持自动图像尺寸检测和分辨率设置，调用订阅接口获取首张图片的完整元数据
- 图像编辑：将目标图 URL 放在首位，合并参考图，调用订阅接口获取编辑结果（仍返回纯 URL）
- 文件上传：通过存储模块上传本地文件，返回可公开访问的 URL，包含完整的错误日志记录

**更新** 重大 API 变更：`generateImage` 函数现在返回 `{ url: string; width: number; height: number }` 完整元数据结构，替代了之前的纯 URL 返回。编辑函数 `editImage` 仍然返回纯 URL。新增动态端点选择机制，支持更灵活的模型调用。

```mermaid
flowchart TD
Start(["进入 generateImage"]) --> CheckRefs{"是否存在参考图？"}
CheckRefs --> |是| EditEndpoint["选择端点: fal-ai/nano-banana-2/edit"]
CheckRefs --> |否| TextEndpoint["选择端点: fal-ai/nano-banana-2"]
EditEndpoint --> Merge["合并基础参数与提示词<br/>可选附加 image_urls<br/>支持分辨率和宽高比设置"]
TextEndpoint --> Merge
...
Parse --> Fallback{"FAL API 返回宽高？"}
Fallback --> |是| Return["返回 { url, width, height }"]
Fallback --> |否| LoadImg["加载图片获取实际尺寸"]
LoadImg --> Return
...
**图表来源**
- [lib/fal.ts:64-67](file://lib/fal.ts#L64-L67)
- [lib/fal.ts:75-109](file://lib/fal.ts#L75-L109)
- [lib/fal.ts:111-153](file://lib/fal.ts#L111-L153)
...
- 增强的日志记录：所有请求都记录详细的时间戳和 URL 信息
- 统一的错误处理：捕获所有异常并返回结构化的错误响应，包含时间戳和错误详情
...
K-->>R : "返回响应"
R-->>F : "返回响应"
Note over R : "增强日志记录：<br/>- 记录请求接收时间<br/>- 记录请求 URL<br/>- 捕获并记录错误"
```

**图表来源**
- [app/api/fal/proxy/route.ts:5-18](file://app/api/fal/proxy/route.ts#L5-L18)
- [app/api/fal/proxy/route.ts:22-46](file://app/api/fal/proxy/route.ts#L22-L46)
...
Upload --> Log["记录上传日志<br/>包含文件名、大小、类型"]
Log --> Done{"成功？"}
...
**图表来源**
- [components/canvas/InlineEditPanel.tsx:114-147](file://components/canvas/InlineEditPanel.tsx#L114-L147)
...
- 增强的错误处理：网络错误和生成失败都有明确的错误提示

**更新** 重大逻辑变更：生成模式下 `generateImage` 返回完整元数据，需要分别处理编辑模式和生成模式的不同返回值类型。新增了基于实际图像尺寸的显示优化和文件命名功能。
...
**图表来源**
- [components/canvas/InlineEditPanel.tsx:207-216](file://components/canvas/InlineEditPanel.tsx#L207-L216)
...
- **更新** 动态端点选择：根据输入参数自动选择最优模型端点，提高生成效率
- **新增** 增强日志记录：详细的调试信息帮助快速定位性能瓶颈
...
- 上传失败
  ...
  - 参考位置：[components/canvas/InlineEditPanel.tsx:137-147](file://components/canvas/InlineEditPanel.tsx#L137-L147)
- 生成失败
  - 现象：占位图被移除并提示错误
  - 排查：检查网络状态、代理路由可用性、模型订阅权限
  - 参考位置：[components/canvas/InlineEditPanel.tsx:279-286](file://components/canvas/InlineEditPanel.tsx#L279-L286)
- 编辑模式不可用
  - 现象：编辑按钮被禁用或无响应
  - 排查：确认目标图已上传完成且 URL 可用，参考图数量与格式符合要求
  - 参考位置：[components/canvas/InlineEditPanel.tsx:159-177](file://components/canvas/InlineEditPanel.tsx#L159-L177)
- **更新** 图像尺寸显示异常
  - 现象：生成的图像显示比例不正确或文件名为空
  - 排查：确认 `generateImage` 返回的元数据结构正确，检查调用方是否正确解构了 `url`、`width`、`height` 字段
  - 参考位置：[components/canvas/InlineEditPanel.tsx:211-216](file://components/canvas/InlineEditPanel.tsx#L211-L216)
- **更新** API 返回值类型错误
...
- **更新** 动态端点选择问题
  - 现象：生成模式或编辑模式调用错误的模型端点
  - 排查：检查 `referenceUrls` 参数是否正确传递，确认端点选择逻辑
  - 参考位置：[lib/fal.ts:64-67](file://lib/fal.ts#L64-L67)，[__tests__/fal.test.ts:27-42](file://__tests__/fal.test.ts#L27-L42)
- **更新** 日志记录问题
  - 现象：调试信息缺失或不完整
  - 排查：确认浏览器控制台日志级别设置，检查代理路由的包装函数是否正常执行
  - 参考位置：[lib/fal.ts:70-73](file://lib/fal.ts#L70-L73)，[app/api/fal/proxy/route.ts:25](file://app/api/fal/proxy/route.ts#L25)
- **新增** API 密钥配置问题
  - 现象：API 密钥无法保存或生效
...
**章节来源**
- [components/canvas/InlineEditPanel.tsx:137-147](file://components/canvas/InlineEditPanel.tsx#L137-L147)
- [components/canvas/InlineEditPanel.tsx:279-286](file://components/canvas/InlineEditPanel.tsx#L279-L286)
- [components/canvas/InlineEditPanel.tsx:159-177](file://components/canvas/InlineEditPanel.tsx#L159-L177)
...
- [lib/fal.ts:64-67](file://lib/fal.ts#L64-L67)
- [app/api/fal/proxy/route.ts:25](file://app/api/fal/proxy/route.ts#L25)
...
**更新** 本次更新实现了多项重大增强：动态端点选择机制、增强的错误处理和日志记录、改进的文件上传功能。`generateImage` 函数现在返回完整的图像元数据（包含 URL、宽度、高度），而非仅返回 URL。这一变更显著改善了图像处理管道的精度和性能，使得应用能够基于实际图像尺寸进行优化显示和文件命名。所有调用方都已适配新的返回值结构，包括画布显示、文件命名和尺寸计算。代理路由也增强了日志记录和错误处理能力，为调试和监控提供了更好的支持。后续可按计划文档扩展用户认证与数据库持久化，进一步完善历史记录与协作能力。
...
- **更新** 利用动态端点选择机制：根据是否存在参考图自动选择最优模型
- **更新** 基于实际图像尺寸的显示优化：利用 FAL 返回的精确尺寸进行比例计算和文件命名
- **新增** 启用详细日志记录：利用增强的调试信息快速定位问题
...
- **更新** 利用动态端点选择机制：根据输入参数自动选择最优模型端点
...
- **更新** 利用增强的日志记录：每个 API 调用都有详细的调试信息
- **新增** 动态端点选择验证：确保模型端点选择逻辑正确
...
**更新** 动态端点选择机制：
- 无参考图时：使用 `fal-ai/nano-banana-2` 纯文本生成模式
- 有参考图时：使用 `fal-ai/nano-banana-2/edit` 图片编辑模式

**更新** 增强的日志记录系统：
- 每个 API 调用都会输出详细的调试信息
- 包含时间戳、请求参数摘要和完整错误对象
- 便于快速定位和解决问题
...
- [lib/fal.ts:68-70](file://lib/fal.ts#L68-L70)
- [lib/fal.ts:64-67](file://lib/fal.ts#L64-L67)
- [lib/fal.ts:70-73](file://lib/fal.ts#L70-L73)
- [app/api/fal/proxy/route.ts:25](file://app/api/fal/proxy/route.ts#L25)
