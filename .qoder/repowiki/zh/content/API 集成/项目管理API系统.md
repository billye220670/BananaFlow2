...
## 更新摘要
**变更内容**
- 新增项目保存API端点的预览图片提取功能
- 实现从Tldraw快照中提取预览图片的复杂逻辑处理
- 数据库模式更新以支持preview_images字段
- 前端项目列表和详情页面的预览图片展示功能
...
- [supabase/schema.sql:26-55](file://supabase/schema.sql#L26-L55)
...
SAVE_PROJECT --> EXTRACT_PREVIEW[提取预览图片]
EXTRACT_PREVIEW --> UPDATE_PROJECT[更新项目元数据]
UPDATE_PROJECT --> READY
WAIT --> AUTO_SAVE
```
...
PAGES[Next.js页面]
COMPONENTS[UI组件]
PREVIEW_THUMBNAIL[预览缩略图组件]
END
subgraph "API层"
ROUTES[API路由]
CONTROLLERS[控制器]
MIDDLEWARE[中间件]
PREVIEW_EXTRACTOR[预览图片提取器]
END
subgraph "业务逻辑层"
SERVICES[业务服务]
VALIDATION[数据验证]
...
END
UI --> PAGES
PAGES --> PREVIEW_THUMBNAIL
PREVIEW_THUMBNAIL --> ROUES
ROUTES --> CONTROLLERS
CONTROLLERS --> PREVIEW_EXTRACTOR
PREVIEW_EXTRACTOR --> PREVIEW_SERVICE
CONTROLLERS --> SERVICES
SERVICES --> VALIDATION
SERVICES --> AUTHENTICATION
SERVICES --> REPOSITORIES
PREVIEW_SERVICE --> REPOSITORIES
...
-extractPreviewImages(snapshot) string[]
-processAndUploadAssets(snapshot, userId) ExtendedSnapshot
}
ProjectService --> AutoSaveMechanism : 使用
ProjectService --> StorageService : 依赖
ProjectService --> PreviewExtractor : 集成
```
...
系统实现了智能的预览图片提取功能，从Tldraw快照中提取高质量的预览图片：

```mermaid
flowchart TD
SNAPSHOT[Tldraw快照] --> EXTRACT[extractPreviewImages函数]
EXTRACT --> FIND_IMAGES[查找所有image shapes]
FIND_IMAGES --> FILTER_ASSETS[过滤有效的asset URL]
FILTER_ASSETS --> CALCULATE_AREA[计算图片面积]
CALCULATE_AREA --> SORT_BY_AREA[按面积降序排序]
SORT_BY_AREA --> REMOVE_DUPLICATES[去重处理]
REMOVE_DUPLICATES --> LIMIT_COUNT[限制为前4张]
LIMIT_COUNT --> RETURN[返回预览图片URL数组]
```

**图表来源**
- [app/api/projects/[id]/save/route.ts:17-67](file://app/api/projects/[id]/save/route.ts#L17-L67)
...
PROCESS_SNAPSHOT --> EXTRACT_PREVIEW[提取预览图片]
EXTRACT_PREVIEW --> UPLOAD_SNAPSHOT[上传快照到存储]
...
项目保存流程确保数据的一致性和完整性，并集成了预览图片提取功能：

```mermaid
sequenceDiagram
participant Client as 客户端
participant SaveAPI as 保存API
participant PreviewExtractor as 预览提取器
...
SaveAPI->>PreviewExtractor : 提取预览图片
PreviewExtractor->>PreviewExtractor : 处理快照中的image shapes
PreviewExtractor->>PreviewExtractor : 计算面积并去重
PreviewExtractor-->>SaveAPI : 返回预览图片URL数组
SaveAPI->>SaveAPI : 处理快照中的base64资产
SaveAPI->>Storage : 上传处理后的快照
Storage-->>SaveAPI : 返回存储路径
SaveAPI->>Database : 更新项目元数据包含预览图片
...
**章节来源**
- [app/api/projects/[id]/save/route.ts:1-250](file://app/api/projects/[id]/save/route.ts#L1-L250)
...
系统实现了复杂的预览图片提取算法，能够从Tldraw快照中智能提取高质量的预览图片：

```mermaid
flowchart TD
INPUT[Tldraw快照] --> STEP1[步骤1: 查找所有image shapes]
STEP1 --> STEP2[步骤2: 过滤有效asset URL]
STEP2 --> STEP3[步骤3: 计算图片面积]
STEP3 --> STEP4[步骤4: 按面积降序排序]
STEP4 --> STEP5[步骤5: 去重处理同一URL只保留最大面积]
STEP5 --> STEP6[步骤6: 限制为前4张图片]
STEP6 --> OUTPUT[输出预览图片URL数组]
```

**算法特点**：
- **智能面积计算**: 基于图片的宽高计算面积，优先选择视觉效果最佳的图片
- **URL验证**: 仅接受http/https协议的有效URL，确保图片可访问性
- **去重机制**: 避免重复图片影响预览效果
- **容量限制**: 最终只保留前4张图片，保证预览性能
...
SERVICE --> SUPABASE
SERVICE --> STORAGE
SERVICE --> PREVIEW_EXTRACTOR
STORAGE --> SUPABASE
AUTH --> JWT
SERVICE --> TYPES
STORE --> TYPES
PREVIEW_EXTRACTOR --> TYPES
```
...
STATE_UPDATE[状态更新]
AUTO_SAVE[自动保存]
PREVIEW_EXTRACT[预览图片提取]
end
subgraph "后端处理"
AUTH_CHECK[认证检查]
DATA_PROCESS[数据处理]
PREVIEW_PROCESS[预览处理]
STORAGE_WRITE[存储写入]
DB_WRITE[数据库写入]
end
subgraph "存储层"
...
AUTO_SAVE --> PREVIEW_EXTRACT
PREVIEW_EXTRACT --> AUTH_CHECK
AUTH_CHECK --> DATA_PROCESS
DATA_PROCESS --> PREVIEW_PROCESS
PREVIEW_PROCESS --> STORAGE_WRITE
...
### 预览图片提取优化
- **早期过滤**: 在提取过程中尽早过滤无效图片
- **内存管理**: 合理的数组和集合使用，避免内存泄漏
- **错误边界**: 完善的异常处理，确保提取过程的稳定性
...
- **问题**: 项目列表中预览图片显示为空
- **原因**: 预览图片提取失败或数据库更新异常
- **解决**: 检查extractPreviewImages函数的日志输出，验证快照数据格式
...
CHECK_CONTEXT --> |预览提取错误| HANDLE_PREVIEW_ERROR[处理预览提取错误]
